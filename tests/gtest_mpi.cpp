...
public:
  ...
  void OnTestIterationStart(const UnitTest & u,
                            int it) override { 
    if (listener_)
  ...
  void OnTestIterationEnd(const UnitTest & u,
                          int it) override {
    if (listener_)
  ...
  void GatherPartResults() {
    ...
    if (rank == 0) {
      ...
      for (int r = 1; r < n_proc; ++r) {
        ...
        auto numResults = static_cast<std::size_t>(count) / sizeof(decltype(remoteInfos)::value_type);
